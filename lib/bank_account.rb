...
class BankAccount
  ...
  def deposit_money(monetary_amount)
    deposit(nil, monetary_amount)
  end
  ...
  def deposit(amount, monetary_amount)
    ...
    @transactions << Transaction.new(date: Date.today, value: monetary_amount) if monetary_amount > Money.new(0)
  end
