...
describe 'Withdrawing from a bank account' do
  example 'withdrawing nothing from a bank account' do
    ...
    account.withdraw(monetary_amount: Money.new(0.00))
  ...
  example 'withdrawing an amount from a bank account' do
    ...
    account.withdraw(monetary_amount: Money.new(50.00))
  ...
  example 'withdrawing a negative amount from a bank account' do
    ...
    expect { account.withdraw(monetary_amount: Money.new(-131.66)) }.to raise_error('Cannot withdraw a negative amount of money')
  end

  example 'becoming overdrawn' do
    overdrawn_account = BankAccount.new(initial_deposit: 100, overdraft_limit: -2000)

    overdrawn_account.withdraw(monetary_amount: Money.new(200.00))
  ...
  example 'becoming overdrawn past the overdraft limit' do
    ...
    expect { overdrawn_account.withdraw(monetary_amount: Money.new(200.00)) }.to raise_error('Account is past its overdraft limit')
  end
